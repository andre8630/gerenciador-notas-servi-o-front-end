...
export function EditOrdens() {
        ...
        const [existingImages, setExistingImages] = useState(location.state?.path || [])
        const port = 3001
        ...
        const [status, setStatus] = React.useState(location.state?.status || "Fazer orçamento")
        ...
        const onSubmit = async data => {
            ...
            try {
            await api.put(`service-order/${location.state.id}/update-service`, productDataForm, {
                headers: {
        ...
        <Container>
            ...
            <form   form noValidate onSubmit={handleSubmit(onSubmit)}>
             <div className="div-1">
                ...
                <Label>Status</Label>
                    <Controller
                        ...
                        render={({ field }) => (
                            ...
                            >
                                <StyledMenuItem  value="Fazer orçamento">Fazer orçamento</StyledMenuItem>
                                <StyledMenuItem value="Orçamento aprovado">Orçamento aprovado</StyledMenuItem>
                                <StyledMenuItem value="Em execução">Em execução</StyledMenuItem>
              ...
              <Input type="text" {...register("name")} placeholder="Nome do cliente" defaultValue={location.state?.name} />

            
         
              <Label>CPF</Label>
              <Input type="number" placeholder="CPF"  {...register("document")} defaultValue={location.state?.document}  />
            
      
              <Label>Endereço</Label>
              <Input  type="text" placeholder="Endereço" {...register("address")} defaultValue={location.state?.address} />
            
            
              <Label>Telefone</Label>
              <Input  type="number" placeholder="Telefone do cliente" {...register("phone")} defaultValue={location.state?.phone} />
           
            <Label>Equipamento</Label>
              <Input  type="text" placeholder="Equipamento" {...register("equipment")} defaultValue={location.state?.equipment} />
            
            
              <Label>Descrição</Label>

              <TextArea className="description" rows="5" cols="10" wrap="soft" placeholder="Descrição" {...register("description")} defaultValue={location.state?.description}/>
            
             
              <LabelPrice>Valor {formatCurrency(location.state?.price)}</LabelPrice>

              
            <InputPrice type="number" {...register("price")} placeholder="Digite o novo valor" />
            ...
            <ButtonPrint 
                ...
                <ButtonCreateOrden style={{ gridColumn: "1 / -1" }} type="submit">Atualizar ordem</ButtonCreateOrden>
            ...
            <div className="print-page" >
                ...
                <div className="print-copy" key={i}>
                    <h3 style={{marginBottom: "15px", marginTop: "10px"}}>N° {location.state?.id}</h3>
                    <h2 style={{marginBottom: "15px"}}>{user?.enterprise_name}</h2>
                    
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", marginBottom: "5px"}}>Email...............: {user?.email}</p>
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", marginBottom: "5px"}}>Telefone..........: {user?.phone_number}</p>
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", marginBottom: "5px"}}>CNPJ...............: {user?.enterprise_document}</p>
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", marginBottom: "5px"}}>Data.................: {new Date().toLocaleDateString()}</p>

                    <hr style={{margin: "15px 0"}}/>

                    <p style={{display: "flex", flexDirection: "row", gap:"40px", marginBottom: "5px"}}><strong>Cliente............:</strong> {location.state?.name}</p>
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", marginBottom: "5px"}}><strong>CPF.................:</strong> {location.state?.document}</p>
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", marginBottom: "5px"}}><strong>Telefone.........:</strong> {location.state?.phone}</p>
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", marginBottom: "5px"}}><strong>Endereço.........:</strong> {location.state?.address}</p>
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", marginBottom: "5px"}}><strong>Equipamento...:</strong> {location.state?.equipment}</p>
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", margin: "10px 0"}}><strong>Descrição:</strong></p>
                    <p className="descricao">{location.state?.description}</p>
                    <p style={{display: "flex", flexDirection: "row", gap:"40px", margin: "10px 0", }}><strong>Valor:</strong> {formatCurrency(location.state?.price)}</p>
