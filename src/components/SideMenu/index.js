...
import { Container, ItemContainer, ListLink, MenuButton, User, H1, Logo } from "./styles"
            ...
            <Container isMenuOpen={isMenuOpen} isMobile={isMobile} >
                ...
                {listLinks.map(item => (
                    <ItemContainer key={item.id} isActive={location.pathname === item.link}>
                        ...
                        <ListLink to={item.link} onClick={() => isMobile && setIsMenuOpen(false)}>
                            {item.label}
                        </ListLink>
                    </ItemContainer>
                ))}
